...
class HostTest : public testing::Test {
    public:
        void SetUp() override {
            ...
            _host = std::make_shared<zkpmq::VirtualHost>("host1", "./data/host1/message/", "./data/host1/host1.db");
            _host->declareExchange("exchange1", zkpmq::ExchangeType::DIRECT, true, false, empty_map);
            _host->declareExchange("exchange2", zkpmq::ExchangeType::DIRECT, true, false, empty_map);
            _host->declareExchange("exchange3", zkpmq::ExchangeType::DIRECT, true, false, empty_map);
...
TEST_F(HostTest, init_test) {
    ...
    zkpmq::MessagePtr msg1 = _host->basicConsume("queue1");
    ASSERT_EQ(msg1->payload().body(), std::string("Hello World-1"));
    zkpmq::MessagePtr msg2 = _host->basicConsume("queue1");
    ASSERT_EQ(msg2->payload().body(), std::string("Hello World-2"));
    zkpmq::MessagePtr msg3 = _host->basicConsume("queue1");
    ASSERT_EQ(msg3->payload().body(), std::string("Hello World-3"));
    zkpmq::MessagePtr msg4 = _host->basicConsume("queue1");
