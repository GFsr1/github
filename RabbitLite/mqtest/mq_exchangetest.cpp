...
zkpmq::ExchangeManager::ptr emp;

class ExchangeTest : public testing::Environment {
    public:
        virtual void SetUp() override {
            emp = std::make_shared<zkpmq::ExchangeManager>("./data/meta.db");
        }
...
TEST(exchange_test, insert_test) {
    ...
    // std::unordered_map<std::string, std::string> map {{"k1", "v1"}, {"k2", "v2"}};
    emp->declareExchange("exchange1", zkpmq::ExchangeType::DIRECT, true, false, map);
    emp->declareExchange("exchange2", zkpmq::ExchangeType::DIRECT, true, false, map);
    emp->declareExchange("exchange3", zkpmq::ExchangeType::DIRECT, true, false, map);
    emp->declareExchange("exchange4", zkpmq::ExchangeType::DIRECT, true, false, map);
