...
zkpmq::ConsumerManager::ptr cmp;

class ConsumerTest : public testing::Environment {
    public:
        virtual void SetUp() override {
            cmp = std::make_shared<zkpmq::ConsumerManager>();
...
void cb(const std::string &tag, const zkpmq::BasicProperties *bp, const std::string &body) {
...
TEST(consumer_test, choose_test) {
    zkpmq::Consumer::ptr cp = cmp->choose("queue1");
