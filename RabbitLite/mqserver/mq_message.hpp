...
namespace zkpmq {
    ...
    using MessagePtr = std::shared_ptr<zkpmq::Message>;
    class MessageMapper {
    public:
        ...
        bool remove(MessagePtr &msg) {
            ...
            // 3. 将序列化后的消息，写入到数据所在文件的指定位置（覆盖原有的数据）
    ...
    public:
        ...
        MessagePtr front() {
            ...
            // 获取一条对手消息：从 _msgs 中取出数据
            MessagePtr msg = _msgs.front();
            _msgs.pop_front();
            // 将该消息对象，向待确认的 hash 表中添加一份，等到收到消息确认后进行删除
    ...
    private:
        ...
        std::unordered_map<std::string, MessagePtr> _durable_msgs; // 持久化消息 hash
        std::unordered_map<std::string, MessagePtr> _waitack_msgs; // 待确认消息 hash
    };
    ...
    public:
        ...
        size_t gettable_count(const std::string &qname) {
            QueueMessage::ptr qmp;
            {
                std::unique_lock<std::mutex> lock(_mutex);
                auto it = _queue_msgs.find(qname);
                if (it == _queue_msgs.end()) {
                    DLOG("获取队列%s待推送消息失败：没有找到消息管理句柄！", qname.c_str());
                    return 0;
                }
                qmp = it->second;
            }
            return qmp->gettable_count();
        }
