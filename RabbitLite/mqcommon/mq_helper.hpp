...
namespace zkpmq {
    ...
    public:
        ...
        bool open(int safe_leve = SQLITE_OPEN_FULLMUTEX) {
            ...
            if (ret != SQLITE_OK) {
                ELOG("创建/打开 sqlite 数据库失败：%s", sqlite3_errmsg(_handler));
                return false;
            }
            return true;
        }
    ...
    public:
        ...
        bool write(const char* body, size_t offset, size_t len) {
            ...
            if (!fs.is_open()) {
                fs.clear(); // 清除错误状态
                fs.open(_filename, std::ios::binary | std::ios::out | std::ios::trunc); // 创建新文件
                if (!fs.is_open()) {
                    ELOG("%s 文件打开失败！", _filename.c_str());
                    return false;
                }
            }
            // 2. 跳转偏移量（若偏移量超出文件大小，会自动扩展文件）
