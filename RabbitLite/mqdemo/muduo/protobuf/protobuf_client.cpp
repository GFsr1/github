...
public:
    ...
    Client(const std::string &sip, int sport)
        ...
        _dispatcher.registerMessageCallback<zkp::TranslateResponse>(std::bind(&Client::onTranslate, this, 
            std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));

        _dispatcher.registerMessageCallback<zkp::AddResponse>(std::bind(&Client::onAdd, this,
            std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
...
private:
    bool send(const google::protobuf::Message *message) {
        if (_conn->connected()) { // 链接状态正常，再发送，否则就返回 false
